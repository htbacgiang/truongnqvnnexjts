...
const CoreValues = () => {
  ...
  return (
    <div 
      className="relative text-white py-12 px-6 mb-5 rounded-[5rem_1rem_5rem_1rem] bg-cover bg-center"
      style={{
        backgroundImage: "url('/images/loi-ich-cua-website.jpg')"
      }}
    >
      {/* Lớp phủ mờ */}
      <div className="absolute inset-0 bg-blue-700 opacity-95 rounded-[5rem_1rem_5rem_1rem]"></div>

      <div className="relative max-w-5xl mx-auto text-center">
        <h2 className="text-2xl font-bold mb-6 text-white">8 lợi ích khi sở hữu website</h2>
        <p className="mb-5 text-base text-white leading-relaxed">
        ...
        <div className="grid grid-cols-1 gap-3">
          {values.map((value, index) => (
            <div
              key={index}
              className="flex items-center rounded-lg transform transition-all duration-300 hover:scale-105 hover:shadow-lg"
            >
              {/* Số thứ tự */}
              <div className="flex-shrink-0 bg-orange-400 text-white text-xl font-bold w-10 h-10 rounded-tl-lg rounded-br-lg flex items-center justify-center shadow-md transition-all duration-300 hover:bg-orange-500 hover:scale-110">
                {value.number}
              </div>
              {/* Nội dung */}
              <div className="flex flex-col items-start ml-2 text-left text-white text-sm md:flex-row p-2 md:items-center">
                <h3 className="font-bold md:mr-2 text-white">{value.title}: </h3>
                <span className="text-white text-base leading-relaxed">{value.details[0]}</span>
              </div>
            </div>
          ))}
        </div>
